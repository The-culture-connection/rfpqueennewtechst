...
export default function TimelineStep({ selected, onChange }: TimelineStepProps) {
  return (
    <div>
      ...
      <p className="font-secondary text-[#4a5568]/80 mb-6">Select your preferred timeline</p>
            ...
            <div className="flex items-start">
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-primary text-[#406da8]">{timeline.label}</h3>
                  {selected === timeline.value && (
                    <svg className="w-6 h-6 text-[#406da8]" fill="currentColor" viewBox="0 0 20 20">
                      <path
                        fillRule="evenodd"
                ...
                <p className="text-sm font-secondary text-[#4a5568]/80 mt-1">{timeline.description}</p>
              </div>
            </div>
          </button>
        ))}
