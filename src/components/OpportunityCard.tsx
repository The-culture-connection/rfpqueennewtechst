...
export default function OpportunityCard({ opportunity, onPass, onSave, onApply }: OpportunityCardProps) {
  ...
  const winRateColor = winRate >= 70 ? 'text-[#ff16a9]' : winRate >= 50 ? 'text-yellow-400' : 'text-[#e7e8ef]/60';
  const winRateBg = winRate >= 70 ? 'bg-[#1d1d1e] border-[#ff16a9]/50' : winRate >= 50 ? 'bg-[#1d1d1e] border-yellow-400/50' : 'bg-[#1d1d1e] border-[#e7e8ef]/30';
  ...
  return (
    <div className="bg-[#1d1d1e] rounded-xl shadow-lg hover:shadow-xl transition-all border border-[#ff16a9]/30 overflow-hidden">
      ...
      <div className="p-6">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              <span className={`px-2 py-1 rounded text-xs font-secondary border ${
                opportunity.type === 'Grant' ? 'bg-[#1d1d1e] text-[#ff16a9] border-[#ff16a9]/50' : 'bg-[#1d1d1e] text-[#ff16a9] border-[#ff16a9]/50'
              }`}>
                {opportunity.type}
              </span>
              {urgencyBadge && (
            ...
            <h3 className="text-xl font-primary text-[#ff16a9] mb-2">
              {opportunity.title}
            </h3>
        ...
        <div className="space-y-2 mb-4">
          {opportunity.amount && (
            <div className="flex items-center text-sm font-secondary">
              <span className="text-[#ff16a9] mr-2">💰</span>
          ...
          <div className="mb-4">
            ...
            {opportunity.description.length > 150 && (
              <button
                ...
                className="text-sm text-[#ff16a9] hover:text-[#ff16a9]/80 font-secondary mt-1 transition-colors"
              >
        ...
        {/* Action Buttons */}
        <div className="flex gap-2 pt-4 border-t border-[#ff16a9]/30">
          <button
            onClick={() => onPass(opportunity.id)}
            className="flex-1 px-4 py-2 bg-[#1d1d1e] text-[#e7e8ef] rounded-lg hover:bg-[#1d1d1e]/80 transition-all font-secondary border border-[#ff16a9]/30"
          >
            Pass
          </button>
          <button
            onClick={() => onSave(opportunity.id)}
            className="flex-1 px-4 py-2 bg-[#ff16a9] text-white rounded-lg hover:bg-[#ff16a9]/80 transition-all font-secondary"
          >
            Save ✨
          </button>
          <button
            ...
            className="flex-1 px-4 py-2 bg-[#ff16a9] text-white rounded-lg hover:bg-[#ff16a9]/80 transition-all font-secondary disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1"
        ...
        {opportunity.url && (
          <div className="mt-3 pt-3 border-t border-[#ff16a9]/30">
        ...
        {/* Refinement Note */}
        <div className="mt-3 pt-3 border-t border-[#ff16a9]/30">
          <p className="text-xs font-secondary text-[#ff16a9]/80 flex items-start gap-2">
